...
4. Count the long-running containers (the interactive `sql-client` service is excluded).
   ```shell
   docker compose ps --status running --quiet \
     coordinator-server tablet-server zookeeper jobmanager taskmanager | wc -l
   ```
   The expected output is `5`. A lower number means that one or more containers failed to start. Run `docker compose ps -a` to identify them. The `sql-client` service may exit because no interactive terminal is attached; this is expected because a new SQL client container is started later with `docker compose run sql-client`.
